...
def bilet(name_of_movie, date_of_movie, name_of_theatre):
    ...
    fon_bel = ImageTk.PhotoImage(Image.open('фон1_белый.png').resize((600, 400)))
    wh = "white"
    ...
    mesto1 = Label(w_bilet, text=f'Мест: ', height=1, width=5, bg=wh, font=('Helvetica', 12), )
    mesto1.place(x=50, y=260)

    summa = Label(w_bilet, text=f'Стоимость билетов: {count * 230}', font=('Helvetica', 12), bg=wh)
    summa.place(x=50, y=235)

    mesto2 = Label(w_bilet, text=f'', height=5, width=30, bg="cyan", font=('Helvetica', 12), anchor="nw")
    mesto2.place(x=100, y=260)

    iconka = ImageTk.PhotoImage(Image.open('icon.png').resize((150, 150)))
    ...
    def b4(filmname, date, kino_teatr):
        ...
        и правилами к-т 'Премьер Зал" """,
            ...
            buy_button = Button(W1, image=buy_image, borderwidth=0, background=bg_color, activebackground=bg_color,
                                command=partial(bilet, filmname, date, kino_teatr))
