...
class MainWindow:
    ...
    def _load_images(self):
        ...
        imgs = {}
        try:
            imgs['main1'] = PhotoImage(file='main1.png')
            imgs['main2'] = PhotoImage(file='main2.png')
            imgs['main3'] = PhotoImage(file='main3.png')
            imgs['r'] = ImageTk.PhotoImage(Image.open('r.png').resize((45, 60)))
            imgs['l'] = ImageTk.PhotoImage(Image.open('l.png').resize((45, 60)))
...
class PosterWindow(BaseWindow):
    ...
    def setup_ui(self):
        ...
        try:
            bg_img = PhotoImage(file='фон афиша.png')
        ...
        for i, (norm, hover, title) in enumerate(posters):
            try:
                img_norm = PhotoImage(file=norm)
                img_hover = PhotoImage(file=hover)
    ...
    def _show_movie_details(self, title):
        ...
        # Фон
        try:
            bg = PhotoImage(file='фон афиша.png')
...
class SeatSelectionWindow(BaseWindow):
    ...
    def _load_images(self):
        ...
        try:
            self.seat_normal = PhotoImage(file='seet.png')
            self.seat_pressed = PhotoImage(file='seet_press.png')
            self.buy_image = PhotoImage(file='buy.png')
        except:
            pass
...
class TicketWindow(BaseWindow):
    ...
    def _load_images(self):
        ...
        try:
            self.bg = ImageTk.PhotoImage(Image.open('фон1_белый.png').resize((600, 400)))
            self.icon = ImageTk.PhotoImage(Image.open('icon.png').resize((150, 150)))
        except:
            pass
...
class ScheduleWindow(BaseWindow):
    ...
    def _load_images(self):
        ...
        for f in poster_files:
            try:
                self.poster_images[f] = ImageTk.PhotoImage(file=f)
            except:
                pass
    ...
    def _create_scroll_area(self):
        ...
        try:
            bg = tk.PhotoImage(file='raspback2.png')
...
class ContactsWindow(BaseWindow):
    ...
    def _load_images(self):
        ...
        for name, (norm, hover) in icons_map.items():
            try:
                self.icons[f'{name}_norm'] = ImageTk.PhotoImage(Image.open(norm).resize((40, 40)))
                self.icons[f'{name}_hover'] = ImageTk.PhotoImage(Image.open(hover).resize((40, 40)))
            except:
                pass

        # Фоновое изображение
        try:
            bg_img = ImageTk.PhotoImage(Image.open('фон афиша.png').resize((1200, 700)))
